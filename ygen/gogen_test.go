...
func TestGoCodeStructGeneration(t *testing.T) {
	tests := []struct {
		...
		inDefinitions    *Definitions
		inGoOpts         GoOpts
		want             GoStructCodeSnippet
		wantErrSubstring string
	}{{
		name: "simple single leaf mapping",
		inStructToMap: &Directory{
			Name: "Tstruct",
		...
		inDefinitions: &Definitions{
			LeafTypes: map[string]map[string]*MappedType{
				"/root-module/tstruct": {
					"f1": {NativeType: "int8", ZeroValue: "0"},
					"f2": {NativeType: "string", ZeroValue: "nil"},
				},
			},
			MappedPaths: map[string][][]string{
				"/root-module/tstruct/f1": {{"f1"}},
				"/root-module/tstruct/f2": {{"f2"}},
			},
		},
		want: GoStructCodeSnippet{
			StructName: "Tstruct",
...
`,
		...
		wantErrSubstring: "field /root-module/tstruct/f1 had an unknown type",
	}, {
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{
				"/module/input-struct": {Name: "InputStruct"},
			},
			LeafTypes: map[string]map[string]*MappedType{
				"/module/input-struct": {
					"u1": &MappedType{NativeType: "InputStruct_U1_Union", UnionTypes: map[string]int{"string": 0, "int8": 1}},
				},
			},
			MappedPaths: map[string][][]string{
				"/module/input-struct/u1": {{"u1"}},
			},
		},
		want: GoStructCodeSnippet{
...
`,
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{
				"/root-module/input-struct/c1": {Name: "InputStruct_C1"},
			},
			LeafTypes: map[string]map[string]*MappedType{
				"/root-module/input-struct": {"c1": nil},
			},
			MappedPaths: map[string][][]string{
				"/root-module/input-struct/c1": {{"c1"}},
			},
		},
		want: GoStructCodeSnippet{
			StructName: "InputStruct",
...
`,
		...
		name:             "nil target struct",
		wantErrSubstring: "cannot create code for nil targetStruct",
	}, {
		...
		wantErrSubstring: "create code with invalid Definitions",
	}, {
		name: "struct with missing Entry",
		inStructToMap: &Directory{
			Name:   "AStruct",
			Fields: map[string]*yang.Entry{},
			Path:   []string{"", "fish"},
		...
		inDefinitions:    &Definitions{},
		wantErrSubstring: "create code for nil *yang.Entry",
	}, {
		name: "struct with missing leaf types",
		inStructToMap: &Directory{
			Name: "AStruct",
			Fields: map[string]*yang.Entry{
				"elem": {
			...
			Entry: &yang.Entry{Name: "elem"},
			Path:  []string{"", "root-module", "a-struct"},
		},
		inDefinitions:    &Definitions{},
		wantErrSubstring: "can't find leaf definitions for struct /elem",
	}, {
		...
		wantErrSubstring: "could not resolve /root-module/a-struct/elem into a defined struct",
	}, {
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{
				"/root-module/b-struct": {},
			},
			LeafTypes: map[string]map[string]*MappedType{
				"/root-module/b-struct": {"list": nil},
			},
			MappedPaths: map[string][][]string{
				"/root-module/b-struct/list": {{"a path"}},
			},
		},
		wantErrSubstring: "struct for /root-module/b-struct/list did not exist",
	}, {
		name: "struct with missing mapping path for field",
		inStructToMap: &Directory{
			Name: "BStruct",
			Fields: map[string]*yang.Entry{
				"list": {
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{
				"/root-module/b-struct/list": {Name: "list-struct-type"},
			},
			LeafTypes: map[string]map[string]*MappedType{
				"/root-module/b-struct": {"list": nil},
			},
		},
		wantErrSubstring: "cannot find mapping path",
	}, {
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{
				"/root-module/q-struct":        {Name: "QStruct"},
				"/root-module/q-struct/a-list": {Name: "QStruct_AList"},
			},
			LeafTypes: map[string]map[string]*MappedType{
				"/root-module/q-struct": {"a-list": nil},
			},
			MappedPaths: map[string][][]string{
				"/root-module/q-struct/a-list": {{"a-list"}},
			},
		},
		want: GoStructCodeSnippet{
			StructName: "QStruct",
...
`,
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{
				"/root-module/tstruct": {Name: "TStruct"},
			...
			LeafTypes: map[string]map[string]*MappedType{
				"/root-module/tstruct": {"listWithKey": nil},
			},
			MappedPaths: map[string][][]string{
...
`,
		...
		name: "wrong number of list keys",
		inStructToMap: &Directory{
			Name: "Tstruct",
			Fields: map[string]*yang.Entry{
				"listWithKey": {
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{
				"/root-module/tstruct/listWithKey": {
					...
					ListAttr: &YangListAttr{
						Keys: map[string]*MappedType{"one": nil, "two": nil},
					},
				},
			},
			LeafTypes: map[string]map[string]*MappedType{
				"/root-module/tstruct": {"listWithKey": {NativeType: "TStruct_ListWithKey"}},
			},
			MappedPaths: map[string][][]string{
				"/root-module/tstruct/listWithKey": {{"list-with-key"}},
			},
		},
		wantErrSubstring: "missing key definitions",
	}, {
		name: "missing key definition",
		inStructToMap: &Directory{
			Name: "Tstruct",
			Fields: map[string]*yang.Entry{
				"listWithKey": {
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{
				"/root-module/tstruct/listWithKey": {
					Name: "TStruct_ListWithKey",
					ListAttr: &YangListAttr{
						Keys: map[string]*MappedType{"one": nil},
					},
				},
			},
			LeafTypes: map[string]map[string]*MappedType{
		...
		wantErrSubstring: "did not find type for key",
	}, {
		name: "invalid type for key",
		inStructToMap: &Directory{
			Name: "Tstruct",
			Fields: map[string]*yang.Entry{
				"listWithKey": {
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{
				"/root-module/tstruct/listWithKey": {
					Name: "TStruct_ListWithKey",
					ListAttr: &YangListAttr{
						Keys: map[string]*MappedType{"keyLeaf": nil},
					},
				},
			},
			LeafTypes: map[string]map[string]*MappedType{
		...
		wantErrSubstring: "invalid key type",
	}, {
		...
		inDefinitions: &Definitions{
			LeafTypes: map[string]map[string]*MappedType{
				"/mod/container-two/container": {"idd": {IsEnumeratedValue: true}},
			},
			MappedPaths: map[string][][]string{
				"/mod/container-two/container/idd": {{"idd"}},
			},
		},
		wantErrSubstring: "invalid empty type for field",
	}, {
		...
		inDefinitions: &Definitions{
			...
			LeafTypes: map[string]map[string]*MappedType{
				"/exmod/root-module/tstruct": {
					"listWithKey": {NativeType: "ListWithKey"},
				},
			},
			MappedPaths: map[string][][]string{
				"/root-module/tstruct/listWithKey": {{"listWithKey"}},
...
`,
		...
		inDefinitions: &Definitions{
			Directories: map[string]*Directory{},
			LeafTypes: map[string]map[string]*MappedType{
				"/foo/container": {"leaf": {NativeType: "string", ZeroValue: `""`}},
			},
			MappedPaths: map[string][][]string{
				"/foo/bar/leaf": {{"bar", "leaf"}},
			},
		},
		inGoOpts: GoOpts{
...
`,
	...
	errsAsSingleErr := func(errs []error) error {
		if len(errs) == 0 {
			return nil
		}
		b := &bytes.Buffer{}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, errs := writeGoStruct(tt.inStructToMap, tt.inDefinitions, tt.inGoOpts, true)

			if diff := errdiff.Substring(errsAsSingleErr(errs), tt.wantErrSubstring); diff != "" {
				t.Fatalf("did not get expected errors, %s", diff)
			}
