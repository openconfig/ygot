...
func TestInstantiatingModule(t *testing.T) {
	tests := []struct {
		name string
		in   *yang.Entry
		want string
	}{{
		name: "simple leaf in a module",
		in: &yang.Entry{
			Name: "leaf",
			Parent: &yang.Entry{
				Name: "module",
			...
			Node: &yang.Leaf{
				Name: "leaf",
				Parent: &yang.Module{
					Name: "module-from-node",
				},
			},
		},
		want: "module-from-node",
	}, {
		name: "leaf within a grouping",
		in: &yang.Entry{
			Name: "leaf",
			Parent: &yang.Entry{
				Name: "container",
		...
		want: "module-from-entry-node",
	}, {
		name: "leaf within an augment",
		in: &yang.Entry{
			Name: "leaf",
			Parent: &yang.Entry{
				Name: "module-from-entry",
			},
			Node: &yang.Leaf{
				Name: "leaf",
				Parent: &yang.Augment{
	...
	for _, tt := range tests {
		if got := instantiatingModule(tt.in); got != tt.want {
			t.Errorf("%s: instantiatingModule(%#v): did not get expected module, got: %v, want: %v", tt.name, tt.in, got, tt.want)
		}
	}
}
