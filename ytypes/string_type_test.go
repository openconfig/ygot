...
func TestValidateString(t *testing.T) {
	tests := []struct {
		...
		{
			desc:       "alternation under an anchored regular expression, valid",
			length:     yang.Uint64Range[0],
			schemaName: "range-any",
			re:         []string{`^ab|cd$`},
			POSIXRe:    []string{`^(ab|cd)$`},
			val:        "cd",
		},
		{
			desc:       "alternation under an anchored regular expression, invalid",
			length:     yang.Uint64Range[0],
			schemaName: "range-any",
			re:         []string{`^ab|cd$`},
			POSIXRe:    []string{`^(ab|cd)$`},
		...
		{
			desc:       "regular expression ending in an escaped $",
			length:     yang.Uint64Range[0],
			schemaName: "range-any",
			re:         []string{`[0-9]+\$`},
			POSIXRe:    []string{`^[0-9]+\$$`},
