...
func toKeyTest(p *gnmipb.PathElem) string {
	k, err := toKey(p)
	if err != nil {
		panic(fmt.Sprintf("can't marshal key: %v", err))
	}
	return k
}
	...
	tests := []struct {
		...
		inT: &TreeNode{
			...
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "test"}): &TreeNode{},
			},
		},
		wantLeaf: true,
	}, {
...
func TestEqual(t *testing.T) {
	...
	tests := []struct {
		...
		inA:  &TreeNode{subtree: map[string]*TreeNode{}},
		inB:  &TreeNode{},
	}, {
		name: "not equal, different length subtrees",
		inA: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "a"}): {leaf: "a"},
			},
		},
		inB: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "a"}): {leaf: "a"},
				toKeyTest(&gnmipb.PathElem{Name: "b"}): {leaf: "b"},
			},
		},
	}, {
		...
		inB: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "a"}): {leaf: "not-equal"},
			},
		},
	}, {
...
func TestFind(t *testing.T) {
	...
	tests := []struct {
		...
		inTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "e"}): &TreeNode{leaf: "eval"},
			},
		},
		...
		inTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "e"}): &TreeNode{goStruct: sgs},
			},
		},
		inPath:   &gnmipb.PathElem{Name: "e"},
		...
		inTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "n", Key: map[string]string{"k": "v"}}): {leaf: "bar"},
			},
		},
		...
		inTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "bar", Key: map[string]string{"foo": "bar"}}): {leaf: "baz"},
			},
		},
		inPath: &gnmipb.PathElem{Name: "bar", Key: map[string]string{"foo": "baz"}},
	}}

	for _, tt := range tests {
		gotNode := tt.inTree.findByKey(toKeyTest(tt.inPath))
...
func TestAddNode(t *testing.T) {
	tests := []struct {
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "eone"}): {leaf: "value"},
			},
		},
	}, {
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "eone", Key: map[string]string{"foo": "bar"}}): &TreeNode{goStruct: &simpleGoStruct{ygot.String("value")}},
			},
		},
	}, {
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "a"}): {leaf: "b"},
			},
		},
	}, {
...
func TestAddAllNodes(t *testing.T) {
	...
	tests := []struct {
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "foo"}): {
					subtree: map[string]*TreeNode{
						toKeyTest(&gnmipb.PathElem{Name: "bar"}): {leaf: "test"},
					},
				},
			},
		},
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "foo"}): {
					subtree: map[string]*TreeNode{
						toKeyTest(&gnmipb.PathElem{Name: "bar", Key: map[string]string{"one": "two"}}): {goStruct: sgs},
					},
				},
			},
		},
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "foo"}): {
					subtree: map[string]*TreeNode{
						toKeyTest(&gnmipb.PathElem{Name: "bar"}): {leaf: "bar"},
					},
				},
			},
		},
		...
		inTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "config"}): {
					subtree: map[string]*TreeNode{
						toKeyTest(&gnmipb.PathElem{Name: "cheese"}): {leaf: "cheddar"},
					},
				},
			},
		},
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "config"}): {
					subtree: map[string]*TreeNode{
						toKeyTest(&gnmipb.PathElem{Name: "bar"}):    {leaf: "baz"},
						toKeyTest(&gnmipb.PathElem{Name: "cheese"}): {leaf: "cheddar"},
					},
				},
			},
...
func TestAddChildrenInternal(t *testing.T) {
	...
	tests := []struct {
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "simple-field"}): {leaf: ygot.String("foo")},
			},
		},
	}, {
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "simple-field"}): {leaf: mps.Field},
				toKeyTest(&gnmipb.PathElem{Name: "config"}): {
					subtree: map[string]*TreeNode{
						toKeyTest(&gnmipb.PathElem{Name: "simple-field"}): {leaf: mps.Field},
					},
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "c"}): {
					goStruct: ns,
					subtree: map[string]*TreeNode{
						toKeyTest(&gnmipb.PathElem{Name: "s"}): {
							leaf: ns.C.L,
						},
					},
				},
		...
		wantTree: &TreeNode{
			subtree: map[string]*TreeNode{
				toKeyTest(&gnmipb.PathElem{Name: "m", Key: map[string]string{"key-one": "one", "key-two": "1"}}): {
					goStruct: ms.M["one"],
					subtree: map[string]*TreeNode{
						toKeyTest(&gnmipb.PathElem{Name: "key-one"}): {leaf: ms.M["one"].KeyOne},
						toKeyTest(&gnmipb.PathElem{Name: "key-two"}): {leaf: ms.M["one"].KeyTwo},
					},
				},
				toKeyTest(&gnmipb.PathElem{Name: "m", Key: map[string]string{"key-one": "two", "key-two": "2"}}): {
					goStruct: ms.M["two"],
					subtree: map[string]*TreeNode{
						toKeyTest(&gnmipb.PathElem{Name: "key-one"}): {leaf: ms.M["two"].KeyOne},
						toKeyTest(&gnmipb.PathElem{Name: "key-two"}): {leaf: ms.M["two"].KeyTwo},
					},
				},
			},
		},
